...
%% 
current_sample=zeros(1, length(message));
C_current_sample=zeros(1, length(message));
...
for snr_i = 1:length(SNR)
    ...
    for i = 1:size(message,2)
        ...
        % Make decision of MF
        current_sample(i) = MF_out(n1 + taw -1);
        if (current_sample > Vth)
            MF_out_decided(i) = 1;
        end
        
         %Make decision of Correlator:
        C_current_sample(i) = c_out_cut(taw *(i-1) +1);
...
%for debgging
    figure
    stem((1:length(C_current_sample)),C_current_sample)
    title("auto current")
    
    figure
    stem((1:length(current_sample)),current_sample)
    title("current")
